...
await p.addStyleTag({content:'.force .pbr--6{display:none !important}'});
for (const w of [1240,1200,1180,1152,1120,1100,1082,1060,1040]){
 await p.setViewportSize({width:w,height:900}); await p.waitForTimeout(160);
 const r=await p.evaluate(()=>{
  document.querySelector('.hero__poster').classList.add('force');
  ...
  const maxR=Math.max(...ks.map(k=>lines[k].r));
  document.querySelector('.hero__poster').classList.remove('force');
  return {n:ks.length, widths:ks.map(k=>+(lines[k].r-lines[k].l).toFixed(0)), air:+(rule.right-maxR).toFixed(1)};
 });
 console.log('vw='+w, '3-line forced ->', JSON.stringify(r));
}
await b.close();
